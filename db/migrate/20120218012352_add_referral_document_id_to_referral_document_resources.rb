class AddReferralDocumentIdToReferralDocumentResources < ActiveRecord::Migration
  def self.up
    ...
    add_index :referral_document_resources, [:referral_document_id, :resource_id], :unique => true, :name => "idx_refdoc_resources_refdoc_id_resource_id"
  end

  def self.down
    remove_index :referral_document_resources, :column => [:referral_document_id, :resource_id], :name => "idx_refdoc_resources_refdoc_id_resource_id"
